...
SELECT 
	sd.first_name, m.subject, m.mark, m.Std
FROM 
	studentdetails sd
		JOIN 
		marks m 
			ON sd.ID = m.ID
WHERE 
	m.academic_year IN (YEAR(NOW()), YEAR(NOW())-1) AND m.mark > 0.9 * (SELECT MAX(mark) FROM marks WHERE academic_year IN (YEAR(NOW()), YEAR(NOW())-1))
ORDER BY 
	sd.first_name;
